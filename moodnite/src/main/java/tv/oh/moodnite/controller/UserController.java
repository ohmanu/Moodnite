...
import tv.oh.moodnite.service.moodnite.WatchedService;
...
public class UserController {
	...
	public String saveUser(@ModelAttribute("user") User user, HttpSession session, Model model) {
		...
		if (user.getPassword().length() < 8) {
			model.addAttribute("password_message", "Password must be at least 8 characters.");
			valid = false;
		}
		if (user.getName().isEmpty() || userService.findByName(user.getName()) != null){
			model.addAttribute("name_message", "Please, try another user name.");
			valid = false;
		}
		if (!valid) {
			return "/user/sign-in";
	...
	/**
	 * Recibe la petición de login de usuario.
	 * 
	 * @param user
	 * @param session
	 * @return
	...
	public String loginUser(@ModelAttribute("user") User user, HttpSession session, Model model) {
		...
		if (loogedInuser == null) {
			model.addAttribute("login_message", "Incorrect user/password.");
			return "/user/login";
		}
	...
	/**
	 * Recibe la solicitud de modificación de datos de usuario.
	 * 
	 * @param user
	 * @param session
	 * @return
	 ...
	 * 
	 * @param file
	 *            Potencial nuevo avatar de usuario.
	 * @param user
	 * @param model
	 * @param session
	 * @return
	...
	public String updateAvatar(@RequestParam("file") MultipartFile file, @ModelAttribute("user") User user, Model model,
			HttpSession session) {
		User loggedInUser = (User) session.getAttribute("loggedInUser");
	...
	/**
	 * Carga el formulario que permite al usuario indicar que ha visto o está
	 * viendo una película.
	 * 
	 * @param movieId
	 *            Id de la película que el usuario solicita marcar como vista.
	 * @param model
	 * @param session
	 * @return
	 */
	...
	/**
	 * Agrega el visionado de una película a la lista de películas vistas del
	 * usuario en sesión.
	 * 
	 * @param movieId
	 *            ID de la película.
	...
	public String watchMovie(@PathVariable String movieId, @ModelAttribute("watch") Watched watch,
			HttpSession session) {
		User loggedInUser = (User) session.getAttribute("loggedInUser");
	...
	/**
	 * Elimina el visionado de una película de la lista de películas vistas del
	 * usuario en sesión.
	 * 
	 * @param watchId
	 *            ID del visionado que el usuario quiere eliminar.
	 * @param session
	 * @return
	 */
	...
	/**
	 * Carga el formulario de valoración en estrellas para una película
	 * determinada por el id de la película.
	 * 
	 * @param movieId
	 * @param model
	 * @param session
	...
	/**
	 * Almacena la valoración en estrellas en la lista de reviews del usuario en
	 * sesión y carga el formulario de review.
	 * 
	 * @param movieId
	 * @param stars
	 * @param model
	...
	/**
	 * Agrega la review a la valoración del usuario en sesión a la película
	 * determinada.
	 * 
	 * @param movieId
	 * @param rate
	...
	/**
	 * Elimina la valoración determinada por el id de la película de la lista de
	 * valoraciones del usuario en sesión.
	 * 
	 * @param movieId
	 * @param session
	 * @return
